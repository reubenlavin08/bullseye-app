...
- [ ] **V17. Settings → location change doesn't apply to existing watches.** Reported 2026-05-10. User changed home location from Vancouver → Los Angeles in Settings, but no LA listings appeared. Root cause: each `user_searches` row has its own `latitude`/`longitude` columns frozen at watch-creation time. Settings POST writes to `user_settings.home_*` but never cascades to `user_searches`. `_load_search()` in `scheduler/jobs.py` reads the watch's stale columns, so polls keep using the Vancouver coords. Fix: `_load_search` LEFT JOINs `user_settings` and prefers the live `home_latitude`/`home_longitude` over the watch's frozen values when set; falls back to the watch row when settings has no home location yet. Single source of truth, no migration, applies on the next coordinator tick.

- [ ] **V18. eBay comps come back in CAD while US listings are in USD.** Reported 2026-05-10. Follow-up to V17 — after switching home to Los Angeles, FB listings come in USD but the eBay comp lookups still hit `EBAY-ENCA` (Canadian eBay, prices in CAD). The score compares apples to oranges: a $200 USD listing gets compared to a CAD distribution where $280 CAD = $200 USD for the same item, so the listing falsely looks 30%+ under-market. Root cause: `region="EBAY-ENCA"` is hardcoded in `scheduler/jobs.py:556` (the hot path), `webapp/app.py:2306,2936,3274` (API defaults), and `static/tab_test.js:104` (Test Appraiser). Fix: add `resolve_ebay_region()` in `deal_finder/cloud/comps.py` that derives the marketplace from `user_settings.home_label`'s country suffix (US → EBAY-US, Canada → EBAY-ENCA, UK → EBAY-GB, etc.) and use it as the default region across all call sites. Existing CAD cache rows aren't migrated; EBAY-US queries become fresh fetches and populate a separate cache key.
